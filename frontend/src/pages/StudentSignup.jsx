  ...
  const submit = async () => {
    ...
    try {
      const data = await apiRequest("/api/auth/signup-student", {
        method: "POST",
        body: JSON.stringify({ email, password, teacherEmail, displayName, learningLanguage }),
      });
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-950">
      <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900/70 p-8 shadow-2xl">
        <div className="mb-6">
          <div className="text-2xl font-semibold text-white">Student Signup</div>
          <div className="text-sm text-slate-400">
            Join your teacher's class and start solving problems in your target language.
          </div>
        </div>
